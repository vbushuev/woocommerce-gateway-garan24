<?php
/**
 * Formats Garan24 order data for creating/updating WC orders
 *
 ...
 * Process shipping
 * - Rest and V2
 * Returns array formatted for Garan24
 *
 */
class WC_Gateway_Garan24_WC2K {
	...
	/**
	 * Check which Garan24 country is in use.
	 *
	 * @since  2.0.0
	 * @access public
	 * @var    boolean
	 */
	public $garan24_country;
	 ...
	 *
	 * @param bool $is_rest
	 * @param string $garan24_country
	 */
	public function __construct( $is_rest = false, $garan24_country = '' ) {
		global $woocommerce;
		$this->cart           = $woocommerce->cart->get_cart();
		$this->is_rest        = $is_rest;
		$this->garan24_country = $garan24_country;
	}
	...
	/**
	 * Formats cart contents for Garan24.
	 *
	 * Checks if WooCommerce cart is empty. If it is, there's no reason to proceed.
	 *
	 * @since  2.0.0
	 * @access public
	 *
	 * @return array $cart_contents Formatted array ready for Garan24.
	 */
	public function process_cart_contents() {
		...
		if ( $woocommerce->shipping->get_packages() ) {
			...
			if ( $this->is_rest ) {
				...
				No need to do this any longer, shipping is sent to Garan24
		...
		if ( $this->is_rest && 'us' == $this->garan24_country ) {
		...
		if ( WC()->cart->applied_coupons ) {
			foreach ( WC()->cart->applied_coupons as $code ) {
				...
				if ( 'yes' != $garan24_settings['send_discounts_separately'] && $coupon->discount_type != 'smart_coupon' ) {
					break;
				}
	 ...
	 *
	 * @return integer $item_tax_rate Item tax percentage formatted for Garan24.
	 */
	...
	public function get_item_price( $cart_item ) {
		...
		$item_price = apply_filters( 'garan24_item_price_including_tax', $item_price_including_tax );
	...
	public function get_item_discount_rate( $cart_item ) {
		...
		if ( 'yes' != $garan24_settings['send_discounts_separately'] && $cart_item['line_subtotal'] > $cart_item['line_total'] ) {
	...
	public function get_fee_tax_amount( $cart_fee ) {
		if ( $cart_fee->taxable && 'us' != $this->garan24_country ) {
	...
	public function get_shipping_name() {
		...
		if ( ! isset( $shipping_name ) ) {
			$shipping_name = __( 'Shipping', 'woocommerce-gateway-garan24' );
		}
	...
	public function get_shipping_reference() {
		...
		if ( ! isset( $shipping_reference ) ) {
			$shipping_reference = __( 'Shipping', 'woocommerce-gateway-garan24' );
		}
	...
	public function get_shipping_tax_rate() {
		...
		if ( $woocommerce->cart->shipping_tax_total > 0 && 'us' != $this->garan24_country ) {
